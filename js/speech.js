...
let bubbles = [];
// let content_url = "../data/stories.json";
let yStart = 0; //starting position of the text wall
let customFont; //variable for custom font
let interval=950; //interval between text
// console.log(content.slice(0,6))
function preload() {
    customFont = loadFont('fonts/HanYiQiHei-25Jian-Regular-2.ttf'); //use  preload to load the custom font
    // contentData=loadJSON(content_url);
    // console.log(contentData[0])
    // let orderL=[]
    for (let i = 0; i < contentData.length; i++) {
        // Put object in array
        //
        // if(contentData[i]["content"].includes("company")){
        //     orderL.push(i+1)
        // }
        bubbles.push(contentData[i]["content"]);
    }
    // console.log(orderL)
}

function setup() {
    const canvasSpeech=createCanvas(400, 900);
    canvasSpeech.parent("#speech-container")
    textFont(customFont); //use the custom font for text display
    textAlign(LEFT, CENTER); //adjust the anchor point of text alignment to the horizontal and vertical centers
    textSize(20); //make the text 20 pixels in size
    textLeading(25);
    fill("#ffffff")

    speech = new p5.Speech(); //callback, speech synthesis object
    ...
    function gotSpeech() {
        ...
        if (speechRec.resultValue) {
            ...
            let a = Math.floor(Math.random() * (bubbles.length + 1));
            // if (said.length>50){
            //     content=content+"\n\n"+bubbles[a]
            // }

            content=content+"\n\n"+bubbles[a]+"\n\n"+said+"\n\n";
            interval=15*Math.floor(content.length/50);
        }
    }
}
...
function draw() {
    ...
    for (let y = yStart; y < height; y += interval) { //use a for loop to draw the line of text multiple times down the vertical axis
        // fill(255, y / 2 + 55, 100); //create a gradient by associating the fill color with the y location of the text
        // textWrap(WORD);
        text(content, 5, y,390,990); //display text
    }
    ...
    // speech.setVoice("Sandy (English (US))");
...
function mousePressed() {
    speech.setVoice('Daniel');
    // speech.setVoice("Sandy (English (US))");
    speech.speak(content); // say something
    console.log(content)
}
